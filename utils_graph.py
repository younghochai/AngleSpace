...
def plot_joint_trajectory(
    x, y, z, joint_idx, joint_name, save_name, min_vals, max_vals, motion_id, tick_interval=None, frame_marks=None):
    ...
    for seg_idx, f in enumerate(frame_marks, start=1):
        ax.scatter(
            x[f], y[f], z[f],
            s=60, marker='D',  # 다이아몬드 마커
            edgecolors='k', facecolors='none', linewidths=1.2
        )
        ax.text(
            x[f], y[f], z[f],
            f"S{seg_idx}",
...
def plot_joint_angles_per_frame(
    ...
    save_name="angles_per_frame", base_path="Results/",
...
def plot_selected_right_joint_axes(
    x, y, z, joint_idx, joint_name, motion_id,
    base_path="Results/", y_axis_range=None,
    axis_labels=None,
):
    """
    오른쪽 힙: X, Z
    ...
    def _plot_single(frames, values, label, color, save_plot_name):
        ...
        save_path = generate_plot_path(
            base_path, motion_id, joint_idx, joint_name,
            save_plot_name,
            "axis"
        )
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
    ...
    if joint_name == "R_Hip":
        # X
        _plot_single(
            frames, x_vals, axis_labels[0], 'r',
            save_plot_name="R_Hip_X"
        )
        # Z
        _plot_single(
            frames, z_vals, axis_labels[2], 'b',
            save_plot_name="R_Hip_Z"
        )

    elif joint_name == "R_Knee":
        _plot_single(
            frames, x_vals, axis_labels[0], 'y',
            save_plot_name="R_Knee_X"
        )

    elif joint_name == "R_Ankle":
        _plot_single(
            frames, x_vals, axis_labels[0], 'g',
            save_plot_name="R_Ankle_X"
        )
    ...
    if all(k in right_leg_cache for k in ("R_Hip", "R_Knee", "R_Ankle")):
        ...
        ax.plot(
            frames_c, hip["x"],
            'r-', linewidth=1.5, label="R_Hip X"
        )
        # R_Hip Z (파랑)
        ax.plot(
            frames_c, hip["z"],
            'b-', linewidth=1.5, label="R_Hip Z"
        )
        # R_Knee X (노랑)
        ax.plot(
            frames_c, knee["x"],
            'y-', linewidth=1.5, label="R_Knee X"
        )
        # R_Ankle X (초록)
        ax.plot(
            frames_c, ankle["x"],
            'g-', linewidth=1.5, label="R_Ankle X"
        )
        ...
        save_path_combined = os.path.join(
            folder_path,
            f"{motion_id}_RightLeg_selected_axes_combined_axis.png"
        )
        plt.savefig(save_path_combined, dpi=300, bbox_inches='tight')
        plt.close()
